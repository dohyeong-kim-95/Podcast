...
### T-040: 팟캐스트 조회 API ✅
- [x] `GET /api/podcasts/today` (오늘 팟캐스트 정보 + signed download URL)
- [x] 상태별 응답: completed → 오디오 URL, generating → 진행 중, failed → 수동 트리거 안내, none → 소스 없음
- [x] `POST /api/podcasts/{id}/downloaded` 엔드포인트 추가
- **의존**: T-032
- **완료 기준**: 팟캐스트 상태별로 올바른 응답 반환 ✅

### T-041: 오디오 플레이어 UI ✅
- [x] `AudioPlayer` 컴포넌트 (HTML5 Audio 기반)
- [x] 재생/일시정지 토글
- [x] 시크바 (드래그 가능, 터치/마우스 지원)
- [x] 배속 조절 (1x / 1.5x / 2x)
- [x] 다운로드 버튼 (mp3 로컬 저장)
- [x] 다운로드 완료 시 `POST /api/podcasts/{id}/downloaded` 호출 → Firestore `downloaded: true` 업데이트
- [x] 메인 페이지(`page.tsx`)에 통합
- **의존**: T-040, T-011
- **완료 기준**: 모바일에서 팟캐스트 재생, 배속 변경, 다운로드 동작 ✅

### T-042: 피드백 기능 ✅
- [x] `FeedbackBar` 컴포넌트 (좋았다 / 보통 / 별로 버튼)
- [x] `POST /api/podcasts/{id}/feedback` API (Phase 3에서 구현 완료)
- [x] Firestore 팟캐스트 문서 `feedback` 필드 업데이트
- [x] 사용자 메모리 `feedbackHistory`에 추가
- **의존**: T-040
- **완료 기준**: 피드백 선택 → Firestore에 반영 확인 ✅

### T-043: 수동 트리거 버튼 ✅
- [x] 메인 페이지에서 팟캐스트 상태가 `failed`일 때 "다시 생성" 버튼 표시
- [x] 버튼 클릭 → `POST /api/generate/me` 호출
- [x] 생성 중 로딩 상태 표시 (스피너 + 5초 폴링으로 자동 갱신)
- **의존**: T-041, T-032
