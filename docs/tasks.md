...
- [ ] ID 토큰 검증 미들웨어 구현 (사용자 API용)
- [ ] 이메일 화이트리스트 체크 (환경변수 `ALLOWED_EMAILS`)
- [ ] Scheduler OIDC 토큰 검증 미들웨어 구현 (내부 API용: `/api/generate`, `/api/remind-download`)
...
- [ ] 당일 팟캐스트 status가 completed인 사용자 스킵 (Scheduler 재시도 시 중복 방지)
- [ ] 소스 0개 체크 → 스킵 + 푸시 알림
- [ ] NB 쿠키 유효성 체크
...
- [ ] 배속 조절 (1x / 1.5x / 2x)
- [ ] 다운로드 버튼 (mp3 로컬 저장)
- [ ] 다운로드 완료 시 `POST /api/podcast/{id}/downloaded` 호출 → Firestore `downloaded: true` 업데이트
