...
### T-000: 모노레포 구조 초기화
- [ ] 프로젝트 루트 생성 (`podcast/`)
- [ ] `backend/` (Python FastAPI) 디렉토리 구조
- [ ] `frontend/` (Next.js) 디렉토리 구조
- [ ] `.gitignore`, `README.md`
...
- [ ] Firebase 프로젝트 생성
- [ ] Firestore 활성화 (Spark 플랜)
- [ ] Firebase Storage 활성화
- [ ] Firebase Auth 활성화 (Google 프로바이더)
- [ ] Firebase Hosting 활성화
- [ ] 서비스 어카운트 키 생성 (백엔드용)
- **완료 기준**: Firebase 콘솔에서 모든 서비스 활성 확인

### T-002: Cloud Run + Cloud Scheduler 기본 설정
- [ ] GCP 프로젝트에 Cloud Run API 활성화
- [ ] Cloud Scheduler API 활성화
- [ ] 서비스 어카운트 생성 (Scheduler → Cloud Run 호출용)
- [ ] Dockerfile 작성 (Python 3.11 + Playwright)
- [ ] 테스트 엔드포인트 (`GET /health`) 배포
...
- [ ] ID 토큰 검증 미들웨어 구현
- [ ] 이메일 화이트리스트 체크 (환경변수 `ALLOWED_EMAILS`)
- [ ] `POST /api/auth/verify` 엔드포인트
- **완료 기준**: 유효한 Firebase ID 토큰 + 화이트리스트 이메일로만 API 접근 가능

### T-011: Firebase Auth 연동 (프론트엔드)
- [ ] Firebase SDK 초기화 (`lib/firebase.ts`)
- [ ] Google OAuth 로그인 페이지
- [ ] 인증 상태 관리 (Context/Provider)
- [ ] 보호 라우트 (미인증 → 로그인 리다이렉트)
...
### T-021: 이미지→PDF 변환
- [ ] Pillow로 이미지 로드
- [ ] reportlab로 PDF 생성 (이미지를 full-page 삽입)
...
- [ ] 업로드 진행률 표시
- [ ] `SourceList` 컴포넌트 (파일명, 시각, 썸네일 미리보기)
- [ ] 소스 삭제 버튼 (확인 다이얼로그)
...
- [ ] 재시도 로직 (최대 2회, status 업데이트)
- **의존**: T-030, T-031
- **완료 기준**: 소스가 있는 상태에서 `/api/generate` 호출 → mp3 생성, Firestore 문서 완성

### T-033: Cloud Scheduler 연결
- [ ] Scheduler 크론잡 생성 (`40 6 * * *`, Asia/Seoul)
- [ ] OIDC 인증으로 Cloud Run 엔드포인트 호출
- [ ] `/api/generate`에 Scheduler 인증 검증 추가
- **의존**: T-032, T-002
- **완료 기준**: 매일 06:40에 자동 트리거 확인 (또는 수동 테스트)
...
- [ ] `POST /api/nb-session/start-auth`
  ...
  - 세션 뷰어 URL 반환
- [ ] 서버에서 Playwright로 원격 브라우저 연결
  - `notebooklm.google.com`으로 네비게이트
  - 로그인 완료 감지 (URL 패턴 / 쿠키 체크)
  - `storage_state` 추출
- [ ] `POST /api/nb-session/complete-auth`
  - Fernet 암호화 → Firestore 저장
  - 만료일 추정 (현재+30일)
  - 상태 "valid" 업데이트
- [ ] `GET /api/nb-session/status` (유효/만료임박/만료)
- **완료 기준**: Browserless 세션에서 Google 로그인 → 쿠키 추출·저장 → status valid
